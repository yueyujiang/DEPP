...
| **gpus**               | gpu ids (default `'[0]'`, **Don't** omit the quotes, i.e. use `gpus=[0]` instead of `gpus=[0]`; if no gpu is available, use `gpus=0`; this version doesn't support multiple temporarily, we will update it later)                                            |
